...
WITH hourly AS (
  SELECT
    username_page,
    EXTRACT(HOUR FROM sent_ts) AS hour,
    EXTRACT(DAYOFWEEK FROM sent_ts) AS dow,
  ...
  WHERE sent_ts >= TIMESTAMP_SUB(CURRENT_TIMESTAMP(), INTERVAL 90 DAY)
...
CREATE OR REPLACE TABLE `of-scheduler-proj.core.caption_fatigue_scores_v1`
CLUSTER BY username_page, fatigue_tier
AS SELECT * FROM `of-scheduler-proj.core.caption_fatigue_scores_v1`;
...
SELECT 
  ...
  COUNT(*) FILTER(WHERE engagement_prediction_score > 0.7) AS high_engagement_captions
...
SELECT 
  ...
  COUNT(*) FILTER(WHERE fatigue_tier = 'BURNT_OUT') AS burnt_out_count
