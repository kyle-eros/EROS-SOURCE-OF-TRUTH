...
WITH base AS (
  ...
  JOIN `of-scheduler-proj.mart.caption_rank_next24_v3_tbl` r USING (caption_hash)
  ...
  LEFT JOIN `of-scheduler-proj.core.v_caption_sentiment_v1` s ON s.caption_hash = c.caption_hash
  LEFT JOIN `of-scheduler-proj.core.v_page_engagement_patterns_v1` ep 
    ON ep.username_std = r.username_page
),
...
MERGE `of-scheduler-proj.core.cooldown_settings_v1` T
USING (
  SELECT "enable_smart_cooldowns" AS setting_key, "true"  AS setting_value, "Master switch for smart cooldown system" AS description UNION ALL
  SELECT "emergency_override_enabled", "true", "Allow emergency cooldown overrides for elite content" UNION ALL
  SELECT "min_cooldown_hours",       "6",    "Absolute minimum cooldown in hours" UNION ALL
  SELECT "max_cooldown_days",        "21",   "Maximum cooldown in days regardless of calculations" UNION ALL
  SELECT "fatigue_threshold_warning", "0.5",  "Fatigue score that triggers warnings" UNION ALL
  SELECT "fatigue_threshold_block",  "0.8",  "Fatigue score that blocks usage"
) S
...
VALUES (S.setting_key, S.setting_value, S.description, CURRENT_TIMESTAMP(), "system");
...
AS (
  EXISTS(
    ...
    WHERE caption_hash = caption_hash_param 
...
CLUSTER BY username_std, performance_tier
AS
SELECT 
  caption_hash,
  username_std,
...
SELECT 
  ...
  COUNTIF(emergency_override_eligible) AS emergency_eligible
FROM `of-scheduler-proj.core.v_smart_cooldowns_v1`
...
SELECT 
  ...
  COUNTIF(available_after <= CURRENT_TIMESTAMP()) AS currently_available,
  COUNTIF(available_after > CURRENT_TIMESTAMP()) AS in_cooldown,
  COUNTIF(last_used_ts IS NULL) AS never_used
FROM `of-scheduler-proj.core.v_smart_cooldowns_v1`;
