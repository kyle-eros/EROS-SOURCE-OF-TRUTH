-- =====================================================
-- ML CONFIGURATION TABLES WITH SEED DATA (FIXED)
-- =====================================================
-- Removed COMMENT syntax which isn't supported in CREATE TABLE
...
CREATE TABLE IF NOT EXISTS `of-scheduler-proj.ops.explore_exploit_config_v1` (
  ...
  min_obs_for_exploit INT64 NOT NULL,
  max_explorer_share FLOAT64 NOT NULL,
  cold_start_days INT64 NOT NULL,
  thompson_sampling_enabled BOOL NOT NULL DEFAULT FALSE,
  ucb_enabled BOOL NOT NULL DEFAULT TRUE,
  decay_factor FLOAT64 NOT NULL DEFAULT 0.95,
  updated_at TIMESTAMP NOT NULL
)
PARTITION BY DATE(updated_at);
...
USING (
  SELECT * FROM UNNEST([
    ...
    STRUCT('bytes_billed_daily', NULL, 10737418240.0, TRUE, FALSE, CURRENT_TIMESTAMP()),
...
-- 7. Cooldown settings (needed by ranker)
...
USING (
  SELECT 168 AS min_cooldown_hours,
         3 AS max_weekly_uses,
         CURRENT_TIMESTAMP() AS updated_at
) S
ON TRUE
WHEN NOT MATCHED THEN INSERT ROW;

-- 8. Scheduler assignments (for RLS)
...
MERGE `of-scheduler-proj.ops.scheduler_assignments_v1` T
USING (
  SELECT * FROM UNNEST([
...
ON T.username_page = S.username_page
WHEN NOT MATCHED THEN INSERT ROW;